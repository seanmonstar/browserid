...
https = require('https'),
http = require('http'),
logger = require('./logging/logging.js').getLogger('bid.primary'),
...
var fetchWellKnown = function (currentDomain, principalDomain, clientCB) {
  ...
  var reqTimeout = setTimeout(function abortReq() {
    req.abort();
    handleProxyIDP('timeout trying to load well-known for ' + currentDomain);
  }, primaryTimeout);
  req.on('response', function onReqResponse() { clearTimeout(reqTimeout); });

  req.on('error', function onReqError(e) {
...
function deepFetchWellKnown(principalDomain, cb, currentDomain, delegationChain) {
  ...
  fetchWellKnown(currentDomain, principalDomain, function onWellKnownFetched(err, unparsedDoc) {
...
exports.checkSupport = function(principalDomain, cb) {
  ...
  var startTime = new Date();
  deepFetchWellKnown(principalDomain, function onDeepWellKnownFetched(err, r) {
    var reqTime = new Date() - startTime;
