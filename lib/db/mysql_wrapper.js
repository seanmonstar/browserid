...
exports.createConnection = function(options) {
  ...
  // a testing feature.  By calling `conn.stall` you can
  ...
  var conn = {
    ...
    _resetConnection: function() {
      if (this.realConn) this.realConn.destroy();
      this.realConn = mysql.createConnection(options);
      this.realConn.on('error', function(e) {
        logger.warn("database connection down: " + e.toString());
      });
    },
    ping: function(conn_cb) {
    ...
    _runNextQuery: function() {
      ...
      if (work.ping) {
        this.realConn.ping(function(err) {
          if (stalled) {
        ...
        this.realConn.query(work.query, work.args, function(err, r) {
    ...
    query: function() {
      ...
      this.realConn.end(cb);
    },
    changeUser: function(options, cb) {
      this.realConn.changeUser(options, cb);
    }
  };
  conn._resetConnection();
  conn.database = conn.realConn.database;
  return conn;
};
