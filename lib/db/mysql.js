...
exports.open = function(cfg, cb) {
  if (conn) throw "database is already open!";
  ...
  if (cfg.create_schema) {
    ...
    var createConn = mysql.createConnection(options);
    createConn.query("CREATE DATABASE IF NOT EXISTS " + database, function(err) {
      if (err) {
        logUnexpectedError(err);
        cb(err);
        return;
      }
      createConn.changeUser({ database: database }, function(err) {
        if (err) {
          logUnexpectedError(err);
          cb(err);
          return;
        ...
        function createNextTable(i) {
          if (i < schemas.length) {
            createConn.query(schemas[i], function(err) {
              if (err) {
                logUnexpectedError(err);
                cb(err);
              } else {
...
exports.closeAndRemove = function(cb) {
  var db_to_remove = conn.database;
  ...
  conn.query("DROP DATABASE " + db_to_remove, function() {
    exports.close(cb);
  });
};
...
exports.forgetIDP = function(domain, cb) {
  conn.query('DELETE FROM idp WHERE domain = ?', [ domain ], cb);
};

exports.updateIDPLastSeen = function(domain, cb) {
  conn.query('INSERT INTO idp (domain, lastSeen) VALUES(?,NOW()) ' +
...
exports.getIDPLastSeen = function(domain, cb) {
  conn.query('SELECT UNIX_TIMESTAMP(lastSeen) AS ts FROM idp WHERE domain = ?', [ domain ],
               function(err, rows) {
                 if (err) cb(err);
...
exports.stageUser = function(email, hash, cb) {
  secrets.generate(48, function(secret) {
    ...
    conn.query('INSERT INTO staged (secret, new_acct, email, passwd) VALUES(?,TRUE,?,?) ' +
  ...
  conn.query(
    ...
    function(err, rows) {
      if (err) {
        ...
        // delete the record
        conn.query("DELETE LOW_PRIORITY FROM staged WHERE secret = ?", [ secret ]);
        cb(null, rows[0]);
      }
    });
}
...
exports.stageEmail = function(existing_user, new_email, hash, cb) {
  secrets.generate(48, function(secret) {
    ...
    // overwrite previously staged users
    conn.query('INSERT INTO staged (secret, new_acct, existing_user, email, passwd) VALUES(?,FALSE,?,?,?) ' +
...
exports.emailLastUsedAs = function(email, cb) {
  conn.query('SELECT type as lastUsedAs FROM email WHERE address = ?', [email], function(err, rows) {
    if (err) {
      cb(err);
...
exports.updateEmailLastUsedAs = function(email, type, cb) {
  if (typeEnum.indexOf(type) === -1) {
    ...
    conn.query('UPDATE email SET type = ? WHERE address = ?', [type, email], cb);
  }
};
...
exports.cancelAccount = function(uid, cb) {
  conn.query("DELETE LOW_PRIORITY FROM email WHERE user = ?", [ uid ], function(err) {
    if (err) return cb(err);
    conn.query("DELETE LOW_PRIORITY FROM staged WHERE existing_user = ?", [ uid ], function(err) {
      if (err) return cb(err);
      conn.query("DELETE LOW_PRIORITY FROM user WHERE id = ?", [ uid ], cb);
    });
  });
};
